...
export default function TurnTable({ entries, total, limit, offset, onPageChange }: Props) {
    ...
    <div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "rgba(255,255,255,0.04)" }}>
              ...
              <th style={thStyle}>Input Cost</th>
              <th style={thStyle}>Output Cost</th>
          ...
          <tbody>
            {entries.length === 0 && (
              <tr>
                <td colSpan={10} style={{ ...tdBase, textAlign: "center", padding: "40px 10px", color: "#475569" }}>
                  No turns found
                </td>
              </tr>
            )}
            {entries.map((e, i) => (
              <tr key={e.messageId ?? i} style={{ background: "transparent" }}
                ...
                <td style={{ ...tdBase, color: "#E2E8F0", fontWeight: 600 }}>
                  {e.totalTokens.toLocaleString()}
                </td>
                <td style={{ ...tdBase, color: "#334155" }}>0</td>
                <td style={{ ...tdBase, color: "#94A3B8" }}>
                ...
                <td style={{ ...tdBase, textAlign: "left", fontFamily: "inherit", color: "#475569" }}>
                  {e.messageCreatedAt ? format(parseISO(e.messageCreatedAt), "MM/dd HH:mm:ss") : "—"}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {totalPages > 1 && (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 8, fontSize: 12, color: "#475569" }}>
          <span style={{ fontSize: 11 }}>
            {offset + 1}–{Math.min(offset + limit, total)} of {total.toLocaleString()} turns
          </span>
      ...
      {total > 0 && totalPages <= 1 && (
        <div style={{ marginTop: 8, fontSize: 11, color: "#334155", textAlign: "right" }}>
          Showing {entries.length} of {total} turns
        </div>
      )}
    </div>
  );
